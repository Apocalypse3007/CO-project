...
class Simulator(object):
    ...
    def Find_Register(self, bin):
        return int(bin, 2)
    def readfile(self, file = sys.argv[-2]):
        with open(file, 'r') as file:
            instructions = file.readlines()
            file.close()
        return instructions
    def mem_hex(sefl, val):
        out = "0x0001"+ "0"*(4-len(hex(val)[2:0]))+ hex(val)[2:0]
        return out
    def memory_string(self):
        string = ""
        for i in range(len(self.memory)):
            string += str(self.mem_hex(i)) + ":" + str(self.memory[i]) + "\n"
        return string
    ...
    def I_Type(self, line, instruction):
        ...
        rd = line[20:25]
        if instruction == "addi":
            self.registers[self.Find_Register(rd)][1] = self.registers[self.Find_Register(rs1)][1] + self.binary_2s(imm)
            self.program_counter += 1
            ...
            mc = self.memory[((self.registers[self.Find_Register(rs1)][1]+self.binary_2s(imm)))-4096]
            # print(mc)
            self.registers[self.Find_Register(rd)][1] = self.binary_2s(mc)
            self.program_counter += 1
        elif instruction == "jalr":
            self.registers[self.Find_Register(rd)][1] = (self.program_counter+1)*4
            self.program_counter = int(self.registers[self.Find_Register(rs1)][1]/4) + int(self.binary_2s(imm)/4)   
    def J_Type(self, line , instruction):
        ...
        immval = int(self.binary_2s(imm)/2)
        if instruction == "jal":
            self.registers[self.Find_Register(rd)][1] = (self.program_counter + 1)*4
            self.program_counter = self.program_counter + immval
    def S_Type(self, line , instruction):
        if instruction == "sw":
            ...
            self.memory[((self.registers[self.Find_Register(rs1)][1]+self.binary_2s(imm)))-4096] = self.registers[self.Find_Register(rs2)][1]
            # self.registers[self.Find_Register(rs2)][1] = twos_complement_to_decimal(str((mem[f"0x000{str(decimal_to_hexadecimal(4*(self.registers[self.Find_Register(rs1)][1]+twos_complement_to_decimal(str(imm)))))}"])[-32:]))
            self.program_counter += 1
